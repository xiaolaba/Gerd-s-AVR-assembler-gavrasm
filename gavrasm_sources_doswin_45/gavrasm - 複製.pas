...
{
  ********************************************************
  * Gerd's AVR-Assembler Version 4.5 as of November 2019 *
  * Free Pascal, source code, (C)2019 by DG4FAC          *
  * New in Version 4.5: See ReadMe.Txt for version info  *
  ********************************************************
}
Program gavrasm;
...
Const
  ...
  asHeader:Array[1..nHeader] Of String=(
    '+------------------------------------------------------------+',
    '| gavrasm gerd''s AVR assembler Version 4.5 (C)2019 by DG4FAC |',
    '+------------------------------------------------------------+');
  lHeader:String='gavrasm Gerd''s AVR assembler version 4.5 (C)2019 by DG4FAC';
  cTab=Char(9);
...
While GetValue And GetBitOp(s,p,bType) Do
  ...
  If GetValue Then
    ...
    Case bType Of
      1:If i2>16 Then
        begin
        If pass=2 Then Error(24,WideString(s),WideString('0..15'));
        end else
        begin 
        e1:=i1*EXP(LN(2)*i2);
        e2:=iMaxInt;
        if e1<=e2 Then
          i:=i1 SHL i2 Else {<<}
          Error(9,IntToWideStr(i1),IntToWideStr(i2));
        End;
      2:i:=i1 SHR i2; {>>}
      3:i:=i1 AND i2; {&}
      4:i:=i1 XOR i2; {^}
